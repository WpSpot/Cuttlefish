...
function cuttlefish_postmetatop( $page = false ) { ?>
	<div class="postmeta-top">
		<?php if ( ! $page && 'post' == get_post_type() ) : ?>
			...
			<a href="<?php echo get_permalink(); ?>"><?php the_time( get_option( 'date_format' ) ); ?></a>
		<?php endif; ?>
...
function cuttlefish_postmetabtm() { ?>
	<div class="postmeta-btm">
		<div class="post-meta-item"><?php
			$tags = ( get_the_tags() ) ? sprintf( __( ' Tags: %s.', 'cuttlefish' ), get_the_tag_list( '', ', ' ) ) : '';
			printf( __( 'Categories: %s.%s', 'cuttlefish' ), get_the_category_list( ', ' ), $tags ); ?>
		</div>
	</div><?php
}
endif;
...
function cuttlefish_more_link_text() {
	return __( 'Read the rest of this entry', 'cuttlefish' );
}
endif;
